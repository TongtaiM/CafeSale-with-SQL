-- Items table
.print This practice made by datayolk
.print reference https://datayolk.net/data/cafe-sale-analysis-project/
...
.print \n 2) Cumulative_sales of each customer
.mode box
select  
  Invoices.customer_id,
  cus.email,
  sum(Invoices.quantity*Items.price) as Cumulative_sales
from 
  Invoices
join 
  Items on Invoices.item_id = Items.item_id
join 
  Customers as cus on Invoices.customer_id = cus.customer_id

group by Invoices.customer_id,cus.email
order by Cumulative_sales DESC;

.print \n 3) Create product_category column
.mode box
select  
  item_id,
  item_name,
...
.print \n 4) quantity sold of each productcategory
.mode box
...
select
  ...
  sum(Invoices.quantity) total_quantity_sold,
  sum(Invoices.quantity)*100/(SELECT sum(quantity) FROM Invoices) as percentage_sold
from 
  Invoices
join 
  ProductCategories as Pro on Invoices.item_id = Pro.item_id
join 
  Items on Invoices.item_id = Items.item_id 
group by Pro.product_category;

.print \n 5) total sales of each week
.mode box
select 
...
.print \n 6) total sales of each week and product category
.mode box
with  ProductCategories as (
  select  
    item_id,
...
select 
  case cast (strftime('%w',Invoices.order_date) as integer)
  when 0 then 'Sunday'
  when 1 then 'Monday'
  when 2 then 'Tuesday'
  when 3 then 'Wednesday'
  ...
  Pro.product_category,
  sum(Invoices.quantity*Items.price) total_sales
from 
  Invoices
join 
  ProductCategories as Pro on Invoices.item_id = Pro.item_id
join 
  Items on Invoices.item_id = Items.item_id 
group by
  weekday,Pro.product_category
order by 
  cast (strftime('%w',order_date) as integer),Pro.product_category;
